...
// 학습 수준 평가 - 목표값 (보수적 기준, 2달 학습 가정)
// v33: 목표값 도달 = 80점 (100점 아님). 점근 곡선이라 100엔 영원히 못 닿음 (완료 상태 없음)
const TARGET_VALUES = {
...
// 대시보드 - 5개 지표를 0-100 점수로 정규화 (점근 곡선 — 100엔 영원히 못 닿음)
// v33: 점수 철학 변경 — "완료(100)는 없다, 끊임없이 보완"을 산식에 인코딩
//   (A) 하드캡(min 100) 제거 → 점근 곡선  asymp(v,T) = 100·v / (v + K·T),  K=0.25
//       · 목표값 도달 = 80점, 그 이상은 둔화하며 100엔 못 닿음 (양치기로 '완료' 불가)
//       · 더 학습할수록 점수는 '항상' 오름 (동결 지점 없음) — 단 증가폭은 점감
//   (C) freshness 가중치 ×0.5 → ×1 복원, 5축 단순 평균 → 방치 시 점수 하락(감쇠축)
//       · 누적 4축은 한 번 쌓이면 안 깎이므로, 지속 보완 압력은 최신성이 담당
//   (③) categoryCount도 동일 곡선 → 5종 전부 써도 80 상한 (기존 '공짜 100축' 해소)
//   효과: 잘 관리된 에이전트 ~82, fresh vs 방치 약 9점차, 이론상 최대도 ~92 (100 불가)
//   주의: 표시용 축 점수는 floor로 정수화 → 어떤 축도 화면에 100 표시 안 됨
const ASYMP_K = 0.25; // 곡선 상수 — 낮출수록 상단이 100에 가까워짐 (목표 도달 점수 = 100/(1+K))
function calcDashboardScore(agent) {
  // 점근 곡선: val=0 → 0, val=target → 80, val→∞ → 100(근접하나 도달 불가)
  const asymp = (val, target) => {
    const v = Math.max(0, val || 0);
    return (100 * v) / (v + ASYMP_K * target);
  };

  const i = asymp(agent.itemCount, TARGET_VALUES.itemCount);
  const c = asymp(agent.contentLength, TARGET_VALUES.contentLength);
  const cat = asymp(agent.categoryCount, TARGET_VALUES.categoryCount);
  const cor = asymp(agent.correctionCount, TARGET_VALUES.correctionCount);
  const f = asymp(agent.recentRate, TARGET_VALUES.recentRate);

  // v33: 5축 단순 평균 (freshness 가중치 복원 ×1). 정밀 float로 합산 후 총점만 round.
  const totalScore = Math.round((i + c + cat + cor + f) / 5);

  // 표시용 축 점수는 floor — 점근값은 항상 100 미만이므로 floor 시 화면에 100 절대 미표시
  return {
    itemScore: Math.floor(i),
    contentScore: Math.floor(c),
    categoryScore: Math.floor(cat),
    correctionScore: Math.floor(cor),
    freshnessScore: Math.floor(f),
    totalScore,
  };
}
  ...
  return (
    <div style={{ textAlign:"left" }}>
      ...
      <div style={{
        ...
        <div style={{ fontWeight:700, color:"#94a3b8", marginBottom:5 }}>📖 평가 지표 (5개 평균 · 점근 곡선, 100 도달 불가)</div>
        <div>• <b style={{ color:"#cbd5e1" }}>학습 항목</b>: 등록된 row 수 (목표 300건 = 80점)</div>
        <div>• <b style={{ color:"#cbd5e1" }}>내용 총량</b>: content 글자수 합계 (목표 80,000자 = 80점)</div>
        <div>• <b style={{ color:"#cbd5e1" }}>카테고리</b>: unique category 개수 (5종 = 80점 상한)</div>
        <div>• <b style={{ color:"#cbd5e1" }}>교정 사례</b>: 교정사례 카테고리 row 수 (목표 60건 = 80점)</div>
        <div>• <b style={{ color:"#cbd5e1" }}>최신성</b>: 최근 30일 내 업데이트 비율 (목표 70% = 80점, 가중치 ×1)</div>
        <div style={{ marginTop:6, color:"#475569" }}>※ 목표 도달=80점, 그 이상은 둔화하며 100엔 못 닿음. 방치 시 최신성이 떨어져 점수 하락 — 완료 상태는 없습니다.</div>
      </div>
    </div>
  );
}
